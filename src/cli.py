...
def train_command(args):
    ...
    try:
        ...
        classifier = SongGenreClassifier(
            data_dir=args.data_dir, random_state=args.random_state
        )
        logger.info("Starting model training...")
        # Run training pipeline
        results = classifier.run_full_pipeline(
            balance_data=args.balance, n_components=args.pca_components
        )
...
def predict_command(args):
    ...
    try:
        ...
        feature_columns = [
            col for col in input_data.columns if col not in ["track_id", "genre_top"]
        ]
...
def evaluate_command(args):
    ...
    try:
        ...
        feature_columns = [
            col for col in test_data.columns if col not in ["track_id", "genre_top"]
        ]
...
Examples:
  ...
  python cli.py evaluate --test-data test_data.csv --model decision_tree
    ...
    predict_parser.add_argument(
        "--input", type=str, required=True, help="Input file (CSV or JSON)"
    )
    predict_parser.add_argument(
        "--output", type=str, required=True, help="Output file for predictions"
    )
    predict_parser.add_argument(
        "--model",
        type=str,
    ...
    eval_parser.add_argument(
        "--test-data", type=str, required=True, help="Test data file (CSV)"
    )
    eval_parser.add_argument(
        "--model",
        type=str,
