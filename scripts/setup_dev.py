...
def main():
    ...
    if not run_command(f"{python_cmd} -m pre_commit install", "Installing pre-commit hooks"):
        sys.exit(1)
    
    # Install commit-msg hook
    if not run_command(f"{python_cmd} -m pre_commit install --hook-type commit-msg", "Installing commit-msg hook"):
        sys.exit(1)
    
    # Run pre-commit on all files
    if not run_command(f"{python_cmd} -m pre_commit run --all-files", "Running pre-commit on all files"):
        print("⚠️  Pre-commit found some issues. Please fix them before committing.")
    
    # Run tests
    if not run_command(f"{python_cmd} -m pytest test_song_classifier.py -v", "Running tests"):
