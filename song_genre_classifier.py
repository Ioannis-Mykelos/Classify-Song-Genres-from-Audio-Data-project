...
from config import get_config, get_data_paths, get_model_config, get_feature_config
...
class SongGenreClassifier:
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        for genre, count in genre_counts.items():
            if count < min_samples:
                logger.warning(f"Genre '{genre}' has only {count} samples (minimum: {min_samples})")
        ...
        feature_columns = [col for col in data.columns if col not in ['track_id', 'genre_top']]
    ...
    def analyze_correlations(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if CONFIG['visualization']['save_plots']:
            output_path = CONFIG['paths']['outputs_dir'] / CONFIG['output_files']['correlation_matrix']
    ...
    def preprocess_data(self, data: pd.DataFrame) -> Tuple[np.ndarray, pd.Series]:
        ...
        logger.info(f"Scaled {self.features.shape[1]} features for {len(self.labels)} samples")
        
        return scaled_features, self.labels
    
    def perform_pca(self, scaled_features: np.ndarray, n_components: int = 6) -> np.ndarray:
        """
    ...
    def _plot_pca_results(self, exp_variance: np.ndarray, cum_exp_variance: np.ndarray):
        ...
        # Scree plot
        bars = ax1.bar(range(len(exp_variance)), exp_variance, 
        ...
        ax2.axhline(y=CONFIG['model']['pca_variance_threshold'], 
        ...
        # Add threshold label
        ax2.text(len(exp_variance) * 0.7, CONFIG['model']['pca_variance_threshold'] + 0.05,
        ...
        # Save plot if configured
        if CONFIG['visualization']['save_plots']:
            output_path = CONFIG['paths']['outputs_dir'] / CONFIG['output_files']['pca_analysis']
            plt.savefig(output_path, dpi=CONFIG['visualization']['dpi'], bbox_inches='tight')
    ...
    def balance_dataset(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for genre in data['genre_top'].unique():
            ...
            if len(genre_data) > min_class_size:
                sampled_data = genre_data.sample(min_class_size, random_state=self.random_state)
            else:
    ...
    def cross_validate_models(self, features: np.ndarray, labels: pd.Series, cv_folds: int = 10) -> Dict[str, float]:
        """
        ...
        logreg = LogisticRegression(random_state=self.random_state, max_iter=1000)
        logreg_scores = cross_val_score(logreg, features, labels, cv=kf, scoring='accuracy')
        ...
        logger.info(f"Decision Tree: {cv_results['decision_tree_mean']:.3f} ± {cv_results['decision_tree_std']:.3f}")
        logger.info(f"Logistic Regression: {cv_results['logistic_regression_mean']:.3f} ± {cv_results['logistic_regression_std']:.3f}")
    ...
    def plot_confusion_matrices(self, results: Dict[str, Any]):
        ...
        """
        fig_size = CONFIG['visualization']['figure_size']
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(fig_size[0] * 1.2, fig_size[1] * 0.8))
        
        # Get unique labels for consistent ordering
        unique_labels = sorted(results['test_labels'].unique())
        
        # Decision Tree confusion matrix
        cm_tree = confusion_matrix(results['test_labels'], results['predictions']['tree'], 
        ...
        cm_logreg = confusion_matrix(results['test_labels'], results['predictions']['logreg'],
                                    labels=unique_labels)
        ...
        # Save plot if configured
        if CONFIG['visualization']['save_plots']:
            output_path = CONFIG['paths']['outputs_dir'] / CONFIG['output_files']['confusion_matrices']
            plt.savefig(output_path, dpi=CONFIG['visualization']['dpi'], bbox_inches='tight')
    ...
    def predict(self, features: np.ndarray, model_name: str = "logistic_regression") -> np.ndarray:
        """
    ...
    def run_full_pipeline(self, balance_data: bool = True, n_components: int = 6) -> Dict[str, Any]:
        """
        ...
        corr_matrix = self.analyze_correlations(data.drop(columns=['genre_top', 'track_id']))
